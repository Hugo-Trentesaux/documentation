...
the desired partner. Then, find the :guilabel:`Purchase Withholdings` table in the
:guilabel:`Accounting` tab.

By default, only one withholding record is allowed for each type. To add new withholding lines of
the same type with different date ranges, click the :icon:`oi-settings-adjust` :guilabel:`(adjust)`
icon to reveal the following fields:
...
.. image:: argentina/l10n-ar-partner-assignation.png
   :alt: Partner witholding assignation.
